...
from geom_utils import IslandWrapper
...
class PackScenario(GenericScenario):
                    ...
                    for unmerged_island in unmerged:
                        aligned_set.append(
                            unmerged_island.pixel_perfect_vert_align(
                                ...
                                self.pixel_perfect_vert_align_mode,
                                self.pixel_perfect_align_target))
    ...
    def normalize_scale(self, islands):
        norm_multiplier_iparam_name = self.cx.params['norm_multiplier_iparam_name']
        norm_group_iparam_name = self.cx.params['norm_group_iparam_name']

        norm_multiplier_iparam_desc = None
        if norm_multiplier_iparam_name is not None:
            norm_multiplier_iparam_desc = self.iparams_manager.iparam_desc(norm_multiplier_iparam_name)
        ...
        norm_space = extract_param(CoordSpace, self.cx.params['normalize_space'])

        if norm_group_iparam_name is None:
            output += non_fixed_islands.normalize(norm_space, norm_multiplier_iparam_desc)

        else:
            norm_group_iparam_desc = self.iparams_manager.iparam_desc(norm_group_iparam_name)
    ...
    def island_set_texel_density(self, space, scale_length, island):
        ...
        island_w = IslandWrapper(island, scale_length=scale_length)
    ...
    def pre_run(self):
        ...
        tdensity_set_before_pack = self.cx.params['tdensity_set_before_pack']
        self.tdensity_value = self.cx.params['tdensity_value'] if tdensity_set_before_pack else 0
        ...
        if self.cx.params.get('pixel_perfect_align'):
            self.pixel_perfect_align_target = extract_param(PixelPerfectAlignTarget, self.cx.params['pixel_perfect_align_target'])
