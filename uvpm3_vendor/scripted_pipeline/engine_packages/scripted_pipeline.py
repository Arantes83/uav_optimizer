...
class GroupInfo:
    ...
    def __init__(self, g_scheme, in_group):
        ...
        if self.overrides is not None:
            ...
            def process_attr(attr_type, attr_name, target_name=None, overridden_func=None):
...
from geom_utils import array_bbox
...
class TexelDensityScenario(GenericScenario):
    ...
    def pre_run(self):
        ...
        self.tex_size = self.cx.params['tdensity_tex_size']
        self.scale_length = self.cx.params['tdensity_scale_length']
        self.tdensity_value = self.cx.params['tdensity_value']

        show_iparam_name = self.cx.params['tdensity_show_iparam_name']
        self.show_iparam_desc = self.iparams_manager.iparam_desc(show_iparam_name)

    def calc_tdensity(self, island):
        return IslandWrapper(island, scale_length=self.scale_length).calc_tdensity(self.tex_size)
    
    def set_tdensity(self, island, tdensity_value, pivot=None):
        return IslandWrapper(island, scale_length=self.scale_length).set_tdensity(self.tex_size, tdensity_value, pivot=pivot).get()
    
    def set_tdensity_to_islands(self, input, tdensity_value):
        output = IslandSet()
        pivot = array_bbox(input).center()
        ...
        for island in input:
            ...
            o_island.set_iparam(self.show_iparam_desc, tdensity_value)
