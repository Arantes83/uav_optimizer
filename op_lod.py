...
def _is_generated_lod_object_name(name, base_name):
    """Return True for Blender object names generated by this LOD stage."""
    pattern = rf'{re.escape(base_name)}_LOD\d+(?:\.\d+)?'
    return bool(re.fullmatch(pattern, name or ""))


def _remove_existing_lod_objects(collection, base_name):
    """Remove previous generated LOD objects for this base from the target collection."""
    objects = getattr(collection, "objects", ())
    stale_objects = [
        obj for obj in list(objects)
        if _is_generated_lod_object_name(getattr(obj, "name", ""), base_name)
    ]
...
def _sync_export_lod_collection(context, lod_collection):
    """Point Engine Export at the latest generated LOD collection."""
    export_props = getattr(context.scene, "uav_export_props", None)
    if export_props is not None:
        export_props.collection_ref = lod_collection
...
class UAV_OT_generate_lods(Operator):
    ...
    def invoke(self, context, event):
        ...
        self._removed_existing_lods = _remove_existing_lod_objects(self._lod_col, self._base_name)
        ...
        _sync_export_lod_collection(context, self._lod_col)
    ...
    def modal(self, context, event):
        ...
        if self._step_idx >= len(self._lod_table):
            ...
            cleanup_msg = (
                f' Replaced {self._removed_existing_lods} previous LOD object(s).'
                if getattr(self, '_removed_existing_lods', 0) else
                ''
            )
            self.report(
                {'INFO'},
